...
def visualize_dense(context,
    ...
    output_entry = cv2.resize(output_entry,
                              dim,
    ...
    var_0 = cv2.resize(output_var[0], dim, interpolation=cv2.INTER_NEAREST)
    var_1 = cv2.resize(output_var[1], dim, interpolation=cv2.INTER_NEAREST)
    var_2 = cv2.resize(output_var[2], dim, interpolation=cv2.INTER_NEAREST)

    weight_0 = cv2.resize(output_weight[0],
                          dim,
                          interpolation=cv2.INTER_NEAREST)
    weight_1 = cv2.resize(output_weight[1],
                          dim,
                          interpolation=cv2.INTER_NEAREST)
    weight_2 = cv2.resize(output_weight[2],
                          dim,
                          interpolation=cv2.INTER_NEAREST)
    ...
    for j in range(0, output_size, vec_interval):
        for i in range(0, output_size, vec_interval):
            ...
            if intensity > 0.4:
                # Black arrow for contour
                cv2.arrowedLine(arrows,
                                pnt_0,
                                pnt_1, (0, 0, 0),
                                vec_thickness + 1,
                                tipLength=0.5,
...
def visualize_dense_softmax(context,
                            output_sla,
                            output_dir,
                            output_entry,
                            output_size=128 * 10,
    ...
    for dir_n in range(dir_N):
        output_dirs[dir_n] = cv2.resize(output_dir[dir_n],
                                        dim,
                                        interpolation=cv2.INTER_NEAREST)
    ...
    #########
    # Create a mask with SLA elements over a threshold intensity
    mask = 255 * (output_sla > 0.0).astype(np.uint8)
    # Extract masked elements from SLA array ('non-elements' are 0)
    sla_masked = cv2.bitwise_and(mask, (255.0 * output_sla).astype(np.uint8))
    # Create BGR heatmap from SLA elements
    sla_masked = cv2.cvtColor(sla_masked, cv2.COLOR_GRAY2BGR)
    ...
    #     output_entry_ = (output_entry_ - man_threshold) / (1.0 - man_threshold)
    #     # Create BGR heatmap from Man elements
    #     man_masked = cv2.cvtColor((255 * output_entry_).astype(np.uint8),
    #                               cv2.COLOR_GRAY2BGR)
    ...
    for j in range(0, output_size, vec_interval):
        for i in range(0, output_size, vec_interval):
            ...
            for mode_idx in range(dir_N):

                dir_prob = output_dirs[mode_idx, j, i]

                ang = (mode_idx + 0.5) * 2. * np.pi / dir_N

                di = np.cos(ang)
                dj = np.sin(ang)

                pnt_1_i = int(round(pnt_0_i + dir_prob * vec_dist_len * di))
                pnt_1_j = int(round(pnt_0_j - dir_prob * vec_dist_len * dj))
                pnt_1 = (pnt_1_i, pnt_1_j)
...
def visualize_dir_label(output_viz,
                        mm_ang_tensor,
                        reduction=3,
                        color=(0, 0, 0)):
    '''
    Overlays directional label on top of output visualization

    Args:
        output_viz: RGB image (H,W,3) representing output.
        mm_ang_tensor: Probability of angle intervals (num_angs, H, W).
        reduction: Interval for sparsifying label (i.e. every 6th element).
        color: RGB code for directional label arrows.

    Returns:
        output_viz with direction label as black arrows.
    '''
    # Extract elements with observed directionality
    num_angs = mm_ang_tensor.shape[0]

    # Reduce entries
    i_max = mm_ang_tensor.shape[1]
    j_max = mm_ang_tensor.shape[2]
    mask = torch.ones_like(mm_ang_tensor, dtype=torch.bool)
    for j in range(0, j_max, reduction):
        for i in range(0, i_max, reduction):
            mask[:, i, j] = 0
    mm_ang_tensor[mask] = 0

    # Remove non-observed directions
    avg_prob = 1 / num_angs
    mask = mm_ang_tensor > avg_prob
    mm_ang_tensor = mask * mm_ang_tensor

    # Extract directions as (N,3) matrix
    # [0]: directional
    angs = torch.nonzero(mm_ang_tensor)

    for idx in range(angs.shape[0]):

        # NOTE Indices switched!
        ang_idx, j, i = angs[idx]
        ang_idx = int(ang_idx.item())
        pnt_0_i = int(i.item())
        pnt_0_j = int(j.item())

        resize_coeff = 5
        pnt_0_i *= resize_coeff
        pnt_0_j *= resize_coeff

        pnt_0 = (pnt_0_i, pnt_0_j)

        ang = ang_idx * (1 / num_angs) + (1 / num_angs)
        ang *= 2. * np.pi

        # NOTE Indices switched!
        ang_prob = mm_ang_tensor[ang_idx, j, i] / torch.sum(mm_ang_tensor[:, j,
                                                                          i])
        ang_prob = ang_prob.item()

        di = np.cos(ang)
        dj = np.sin(ang)

        vec_dist_len = 50
        pnt_1_i = int(round(pnt_0_i + ang_prob * vec_dist_len * di))
        pnt_1_j = int(round(pnt_0_j - ang_prob * vec_dist_len * dj))
        pnt_1 = (pnt_1_i, pnt_1_j)

        arrows = copy.deepcopy(output_viz)
        ...
        cv2.addWeighted(arrows, alpha, output_viz, 1 - alpha, 0, output_viz)
